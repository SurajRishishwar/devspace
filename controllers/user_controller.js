...
module.exports.updatepass=async function(req,res){
    try{
        ...
        if(userstoken){
            ...
            return res.render('dev_sign',{
                title:"Sign In",
                isAdded:"Access Denied!! Try After 15 Minutes"
            })
        }
...
module.exports.update=async function(req,res){
    // let upuser=await users.findById(req.body.id);
    // upuser.name=req.body.name;
    // upuser.save();
    // let profile=await users.findById(upuser.id);
    // // console.log(profile);
    // let feed=await post.find({user:upuser.id}).populate('user');
    // return res.redirect('back');
